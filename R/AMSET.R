...
#authenticate with google using your credentials
#bq_auth('hxu5@snapchat.com',scopes=scopes,cache=TRUE,use_oob=TRUE)

# setwd("~/Hui Internship Project/R")
...
simulate <- function(iter,mu,p,m,max_stages,alpha=0.05, tau = 2, mu_fixed){
  ...
  total_obs_or <- 0
  total_obs_dd <- 0
  total_obs_or_always <- 0 
  total_obs_dd_always <- 0
  ...
  for(stage in 1:max_stages){
    ...
    or_remaining <- which(dec_or==0)
    total_obs_or <- total_obs_or+length(or_remaining)
    if(length(or_remaining)!=0){
      T_or_subset <- T_or[stage,or_remaining]
      #rejection
      sorted_T_or_subset <- sort(T_or_subset,index.return=TRUE)
      dec_or[or_remaining[sorted_T_or_subset$ix[which(cummean(sorted_T_or_subset$x)<=alpha)]]] <- 1
    }
    ## approximation using universal inference with estimated proportion
    dd_remaining <- which(dec_dd==0)
    total_obs_dd <- total_obs_dd+length(dd_remaining)
    if(length(dd_remaining)!=0){
      T_dd_subset <- T_dd[stage,dd_remaining]
      #rejection
      sorted_T_dd_subset <- sort(T_dd_subset,index.return=TRUE)
      dec_dd[dd_remaining[sorted_T_dd_subset$ix[which(cummean(sorted_T_dd_subset$x)<=alpha)]]] <- 1
    }
    ## oracle procedure using always valid p-value and BH
    total_obs_or_always <- total_obs_or_always + m
    LR_or_always[stage,] = f1[stage,]/f0[stage,]
    
    ## data driven procedure using always valid p-value and BH
    total_obs_dd_always <- total_obs_dd_always + m
  ...
  if(mu_fixed == TRUE){
    mu = mean(mu)
    return(tibble(iter,m,p,mu,max_stages,alpha,tau,fpr_or,mdr_or,fpr_dd,mdr_dd,fpr_or_always,mdr_or_always,fpr_dd_always,mdr_dd_always,total_obs_or,total_obs_dd,total_obs_or_always,total_obs_dd_always))
  }
  if(mu_fixed == FALSE){
    return(tibble(iter,m,p,max_stages,alpha,tau,fpr_or,mdr_or,fpr_dd,mdr_dd,fpr_or_always,mdr_or_always,fpr_dd_always,mdr_dd_always,total_obs_or,total_obs_dd,total_obs_or_always,total_obs_dd_always))
  }
}
...
# Setting 3: Vary signal proportion from 0.02 to 0.2, random mu from 1.6 to 3 for each coordinate
p <- seq(0.01, 0.1, 0.01)
grid <- expand.grid(iter, p)
res <- pbmcmapply(simulate,
                  iter=grid[,1],
